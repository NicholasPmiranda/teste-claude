...
import { CalendarDays, Clock, ArrowUp, ArrowDown, ArrowRight, AlertCircle } from 'lucide-vue-next';
...
const PriorityIcon = getPriorityIcon(props.priority);
...
const breadcrumbs = [
  ...
  {
    title: getPriorityLabel(props.priority),
    href: route('projects.tasks.by-priority', [props.project_id, props.priority])
  }
];
</script>

<template>
  <Head :title="getPriorityLabel(priority) + ' - Tarefas'" />
    ...
    <div class="flex h-full flex-1 flex-col gap-4 p-4">
      ...
      <div class="flex justify-between items-center mb-6">
        <div class="flex items-center gap-2">
          <h1 class="text-2xl font-semibold text-foreground">Tarefas com Prioridade:</h1>
          <span :class="`inline-flex items-center gap-1 text-sm px-3 py-1 rounded-full ${getPriorityClass(priority)}`">
      ...
      <div v-if="tasks.length === 0" class=" overflow-hidden shadow-sm sm:rounded-lg p-6 text-center ">
        <p>Nenhuma tarefa encontrada com a prioridade "{{ getPriorityLabel(priority) }}".</p>
      </div>
